...
class OSDFace_test(nn.Module):
    ...
    def forward(self, lq, filename):
        if self.use_uni:
            mq_pred = self.uni_model(lq)
        else:
            if filename[4:8] in ANGLES_EXTREME:
                hq_e_pred = self.efr_model(lq)
                mq_pred = self.e2f_model(hq_e_pred, lq)  # reversed order
            elif filename[4:8] in ANGLES_MODERATE:
                mq_pred = self.m2f_model(lq)
            else:
                mq_pred = self.m2f_model(lq)  # use M2F model instead
                # raise "Exception: unrecognized pose in filename: %s" % filename[4:8]

        # 512*512 크기로 리사이징 및 [-1, 1] 범위로 정규화
        mq_resized = Fun.interpolate(mq_pred, size=(512, 512), mode="bicubic")
        mq_resized = (mq_resized - 0.5) * 2.0

        prompt_embeds = self.img_encoder(mq_resized).reshape(lq.shape[0], 77, -1)
        if not self.args.cat_prompt_embedding:
            prompt_embeds = self.embedding_change(prompt_embeds)
        prompt_embeds = Fun.normalize(prompt_embeds, dim=-1)

        lq_resized = (
            Fun.interpolate(lq, size=(512, 512), mode="bicubic") * 2.0 - 1.0
        )  # [-1, 1] 범위로 정규화
        lq_latent = (
            self.vae.encode(lq_resized.to(dtype=self.weight_dtype)).latent_dist.sample()
        ...
        return output_image.clamp(0.0, 1.0), mq_pred
...
def main_worker(Unet, rank, gpu_id, image_names, weight_dtype, args):
    ...
    for image_name in tqdm(image_names):
        ...
        with torch.no_grad():
            ...
            output_image, mq_pred = model(lq, os.path.basename(image_name))

            cr_pil = transforms.ToPILImage()(mq_pred[0].cpu())
