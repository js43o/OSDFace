...
class VectorQuantizer(nn.Module):
    ...
    def __init__(self, args, n_e, e_dim, beta, n_min, output_e_dim=256, latent_w=16):
        ...
        self.embedding = nn.Embedding(self.n_e, self.e_dim)  # 코드북
    ...
    def forward(self, z):
        ...
        return (
            z_q,
            (perplexity, min_encodings, sorted_indices, d),
    ...
    def inference_cat(self, z):
        ...
        d = (
            torch.sum(z_flattened**2, dim=2, keepdim=True)
            + torch.sum(self.embedding.weight**2, dim=1)
            - 2 * torch.matmul(z_flattened, self.embedding.weight.t())
        )  # (batch_size, h*w, n_e)
        ...
        for i in range(batch_size):
            ...
            position_index = torch.arange(0, self.latent_w**2).to(
                z.device, dtype=torch.int64
            )
            position_index = position_index[sorted_order][: self.n_min]
            sorted_indices = sorted_indices[sorted_order]
            indices_tensor = sorted_indices[: self.n_min, :1].to(
                z.device, dtype=torch.int64
            )
        ...
        z_q = torch.matmul(
            min_encodings.view(-1, self.n_e), self.embedding.weight
        )  # (batch_size * h * w, channel)
        if self.args.use_pos_embedding:
            assert (
                not self.args.learnable_pos_emb
            ), "without attention pool, pos embedding shouldn't be learnable"
            pos_z_q = torch.matmul(
                pos_embeddings.view(-1, self.latent_w**2), self.pos_embedding
            )
            z_q = z_q + pos_z_q
    ...
    def inference(self, z):
        ...
        # Perform non-repetitive selection for each image's (h*w, n_e) range
        # ✅ 거리가 가장 가까운 코드북 토큰 77개(= n_min)를 선별
        for i in range(batch_size):
            seen = set()
            selected_indices = []
            min_values, min_indices = torch.min(d, dim=2)
            ...
            indices_tensor = torch.tensor(selected_indices, dtype=torch.int64).to(
                z
            )  # Convert to tensor
            min_encodings[i, : len(selected_indices)].scatter_(
                1, indices_tensor.unsqueeze(1).to(torch.int64), 1
            )
    ...
    def inference_time(self, z):
        ...
        min_encodings = torch.zeros(batch_size, self.n_min, self.n_e).to(z)
        end_time = time.perf_counter()
        timings["Initialize one-hot encoding matrix"] = (
            end_time - start_time
        ) * 1000  # 转换为毫秒
    ...
    def __init__(
        ...
        **ignore_kwargs
        ...
        self.quantize = VectorQuantizer(
            args, n_embed, embed_dim, beta=0.25, n_min=n_min, latent_w=self.latent_w
        )
