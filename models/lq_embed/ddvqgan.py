...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, z):
        ...
        # 코드북 손실(인코더 출력 특징과 가장 가까운 코드북 벡터와의 거리) 계산
        ...
        return (
            z_q,
            loss,
            (perplexity, min_encodings, min_encoding_indices, d),
...
class VQVAEGANMERGE(nn.Module):
    def __init__(
        ...
        **ignore_kwargs
        ...
        self.HQ_encoder = MultiHeadEncoder(
            ch=ch,
            out_ch=out_ch,
            ch_mult=ch_mult,
            num_res_blocks=num_res_blocks,
        ...
        # For ground truth
        self.Fixed_encoder = MultiHeadEncoder(
            ch=ch,
            out_ch=out_ch,
            ch_mult=ch_mult,
            num_res_blocks=num_res_blocks,
...
class DualDecoderVQGAN(nn.Module):
    def __init__(
        self,
        checkpoint,
        embed_dim=512,
        ...
        self.decoder_r = MultiHeadDecoder(
            ch,
            out_ch,
            ch_mult,
            num_res_blocks,
        ...
        self.decoder_f = MultiHeadDecoder(
            ch,
            out_ch,
            ch_mult,
            num_res_blocks,
