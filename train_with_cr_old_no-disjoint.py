...
from dataset_multipie import MultiPIEDataset, ANGLES_EXTREME
...
def parse_args():
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="experiments/02_no-identity-disjoint_use-uni",
...
def main():
    ...
    # Define CR modules
    if args.use_uni:
        uni_model = CoarseRestorer(res=128).to(device)
        uni_model.load_state_dict(load_file(args.ckpt_uni))
        uni_model.eval()
    else:
        m2f_model = CoarseRestorer().to(device)
        efr_model = CoarseRestorer().to(device)
        e2f_model = DualEncoderCoarseRestorer().to(device)

        m2f_model.load_state_dict(load_file(args.ckpt_m2f))
        efr_model.load_state_dict(load_file(args.ckpt_efr))
        e2f_model.load_state_dict(load_file(args.ckpt_e2f))
    ...
    for epoch in range(args.max_epoch):
        for idx, (lq, gt, filename) in enumerate(dataloader):
            ...
            for b in range(lq_resized.shape[0]):
                is_extreme = filename[b][4:8] in ANGLES_EXTREME
                if args.use_uni:
                    cr_out = uni_model(lq_resized[b].unsqueeze(0))
                else:
                    if is_extreme:
                        efr_out = efr_model(lq_resized[b].unsqueeze(0))
                        cr_out = e2f_model(efr_out, lq_resized[b].unsqueeze(0))
                    else:
                        cr_out = m2f_model(lq_resized[b].unsqueeze(0))
                cr_out_list.append(cr_out)
