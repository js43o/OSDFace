...
class OSDFace_test(nn.Module):
    ...
    def load_ckpt(self, ckpt_path):
        ...
        # 🔥 MQ & LQ Fusion Module
        self.fusion_adapter = LatentResidualAdapter()
        self.fusion_adapter.load_state_dict(
            torch.load(
                os.path.join(ckpt_path, "fusion_adapter.pth"),
                weights_only=False,
            )
        )
        self.fusion_adapter.to(self.device, dtype=self.weight_dtype)
    ...
    def forward(self, lq, filename):
        ...
        with torch.cuda.stream(stream2):
            ...
            lq_latent = (
                self.vae.encode(lq_512.to(self.weight_dtype)).latent_dist.sample()
                * self.vae.config.scaling_factor
            )

            # 🔥 fused MQ & LQ
            fused_latent = self.fusion_adapter(mq_latent, lq_latent)

        torch.cuda.synchronize()

        model_pred = self.unet(
            fused_latent, self.timesteps, encoder_hidden_states=prompt_embeds
        ).sample
