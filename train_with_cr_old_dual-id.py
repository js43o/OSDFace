...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.75,
        help="Ratio of MQ-ID loss related to GT-ID loss",
    )
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="experiments/10_dual-id",
...
def main():
    ...
    for epoch in range(args.max_epoch):
        for idx, (lq, gt, filename) in enumerate(dataloader):
            ...
            loss_id_mq = criterion_id(restored_feature, mq_feature, ID_TARGET)
            loss_id_gt = criterion_id(restored_feature, gt_feature, ID_TARGET)
            loss_id = args.alpha * loss_id_mq + (1.0 - args.alpha) * loss_id_gt

            # Consistency Loss
            loss_cons = (
                criterion_mse(restored_img, gt)
                + criterion_perceptual(restored_img, gt)
                + loss_id * args.lambda_id
            )
